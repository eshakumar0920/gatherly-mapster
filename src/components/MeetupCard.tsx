...
const MeetupCard = ({ meetup }: MeetupCardProps) => {
  ...
  const [creatorInfo, setCreatorInfo] = useState<{name: string, id?: string}>({
      ...
      if (user?.email) {
        try {
          ...
          if (!error && userData) {
            ...
            if (eventData && userData.id === eventData.creator_id) {
              setIsCreator(true);
            } else {
              setIsCreator(false);
            }
  ...
  const handleUpdateSuccess = (updatedMeetup: Meetup) => {
    // Update the meetup in the parent component by creating a custom event
    const updateEvent = new CustomEvent('meetup-updated', { 
      detail: { meetupId: meetup.id, updatedMeetup } 
    });
